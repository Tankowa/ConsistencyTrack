...
class ConsistencyTracker(object):
    ...
    def update(self,cur_image):
        ...
        if self.frame_id==1:
            ...
            high_track_inds=consistency_ref_detections[:,4]>self.association_thresh
            consistency_ref_detections,consistency_track_detections=consistency_ref_detections[high_track_inds],consistency_track_detections[high_track_inds]
            
            detections=self.consistency_det_filt(detections,conf_thre=self.low_det_thresh,nms_thre=self.nms_thresh_2d)
            consistency_ref_detections,consistency_track_detections=self.consistency_track_filt(consistency_ref_detections,
                                                                                          consistency_track_detections,
                                                                                          conf_thre=self.low_det_thresh,
                                                                                          nms_thre=self.nms_thresh_2d)
            
            pred_track_ids,pred_bboxes,pred_scores=self.consistency_matching(ref_bboxes,ref_track_ids,
                                                                           consistency_ref_detections,
                                                                           consistency_track_detections)
        ...
        if (self.frame_id-1)%self.re_association_interval==0:
            if self.frame_id!=1:
                ...
                consistency_outputs,conf_scores,association_time=self.consistency_model.new_ddim_sample(inps,images_whwh,num_timesteps=self.sampling_steps,num_proposals=self.num_boxes,
                                                                                                    ref_targets=bboxes,dynamic_time=self.dynamic_time,track_candidate=self.repeat_times,consistency_t=self.re_association_t)
                # self.total_time+=association_time
                consistency_ref_detections,consistency_track_detections,_=self.consistency_postprocess(consistency_outputs,
                                                                                                 conf_scores,
                ...
                pred_track_ids,pred_bboxes,pred_scores=self.consistency_matching(ref_bbox,ref_track_ids,
                                                                consistency_ref_detections,
                                                                consistency_track_detections)
    ...
    def extract_mean_track_t(self,pre_box,cur_box):
        ...
        return min(max(int(track_t*40),1),39)
    ...
    def consistency_matching(self,ref_bboxes,ref_track_ids,consistency_pre_track_outputs,consistency_cur_track_outputs):
        ref_bboxes=np.array(ref_bboxes)
        dists=matching.iou_distance(ref_bboxes,consistency_pre_track_outputs[:,:4])
        matches,u_track, u_detection = matching.linear_assignment(dists, thresh=self.same_thresh)
        if len(matches)>0:
            ref_track_ids=np.array(ref_track_ids)[matches[:,0]]
            return ref_track_ids,consistency_cur_track_outputs[matches[:,1],:4],consistency_cur_track_outputs[matches[:,1],5]
        else:
