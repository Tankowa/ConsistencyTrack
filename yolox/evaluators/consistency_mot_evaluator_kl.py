...
from yolox.tracker.consistency_tracker_kl import ConsistencyTracker
...
class consistencyMOTEvaluatorKL:
    """
    ...
    def evaluate(
        ...
        tracker = ConsistencyTracker(model,tensor_type)
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if frame_id == 1:
                    # text_path="consistencyTrack_outputs/yolox_x_consistency_track_mot20/track_results_mot20_test/{}.txt".format(video_name)
                    # scale = min(
                    ...
                    detections=None
                    tracker = ConsistencyTracker(model,tensor_type,self.confthre,self.detthre,self.nmsthre3d,self.nmsthre2d,self.association_interval,detections)
                    if len(results) != 0:
                ...
                for t in output:
                    ...
                    # save results
                # image_path=os.path.join("consistencyTrack/datasets/dancetrack/train",info_imgs[4][0])
                # raw_image= cv2.imread(image_path)
                # online_im = plot_tracking(
                #     raw_image, online_tlwhs, online_ids, frame_id=frame_id, fps=30
                # )
                # os.makedirs("consistencyTrack/vis_fold/{}".format(video_name),exist_ok=True)
                # cv2.imwrite("consistencyTrack/vis_fold/{}/{:0>5d}.jpg".format(video_name,frame_id),online_im)
        ...
        print("consistency track fps : {}".format(2*n_samples/track_time))
